...
export type TreeSortMode = "name" | "updated";
...
export function buildTree(files: FileEntry[], sortMode: TreeSortMode = "name"): TreeNode {
  if (files.length === 0) {
  ...
  // Sort: directories first, then by sortMode (alphabetical or updated)
  sortTree(root, sortMode);
...
function sortTree(node: TreeNode, sortMode: TreeSortMode): void {
  node.children.sort((a, b) => {
    const aIsDir = a.file == null;
    const bIsDir = b.file == null;
    if (aIsDir !== bIsDir) return aIsDir ? -1 : 1;
    if (sortMode === "updated" && a.file != null && b.file != null) {
      const ta = a.file.updatedAt ? Date.parse(a.file.updatedAt) : 0;
      const tb = b.file.updatedAt ? Date.parse(b.file.updatedAt) : 0;
